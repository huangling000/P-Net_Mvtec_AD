...
class MLNetModel(nn.Module):
    def __init__(self, args, ablation_mode=4):
        ...
        model_G2 = MultiLevelNet(in_ch=3, modality=self.args.data_modality, ablation_mode=ablation_mode,
    ...
    def forward(self, image):
        ...
        image_rec = self.model_G2(image, edge)
...
class RunMyModel(object):
    def __init__(self):
        ...
        # self.acc_top20 = AverageMeter()
        # self.acc_last20 = LastAvgMeter(length=20)
    ...
    def validate_cls(self):
        ...
        with torch.no_grad():
            ...
            self.vis.plot_single_win(dict(value_0=auc,
                                          value_1=auc_list[4],
                                          value_2=auc_list[5],
                                          value_3=auc_list[6],
                                          value_m1=auc_list[2],
            ...
            #     format(self.best_acc, self.acc_last20.avg, self.acc_last20.std)
            ...
            self.vis.text(auc_list, name='auc_list')
        ...
        for i, (image, image_name_item, mask) in enumerate(loader):
            ...
            if category != 'train_normal' and i == 0:
                ...
                for n in range(self.args.vis_batch):
                    self.vis.plot_histogram(image_diff[n].mean(dim=0).view(-1), win='{}_{}'.format(category, n), numbins=500)
                    if n > 3:
                        break
        # self.train_diff_mean /= total_num
